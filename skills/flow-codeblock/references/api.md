...
Common `error.details` fields are `field`/`reason`, `retryAfter` for rate limiting, `remainingAttempts` for verification codes, and `limitInfo` when available. Token rate-limit errors use `TokenRateLimitError` and include the rate-limit headers.
...
| ScriptOwnerError | Caller is not the script owner | 403 |
| ScriptOwnershipClaimedError | Ownership is still claimed, so deletion is forbidden | 409 |
| ScriptOwnershipNotClaimedError | No releasable or manageable owner exists | 409 |
| ScriptOwnershipTransferNotReadyError | Transfer is invalid, expired, or missing a prerequisite | 409 |
...
`id`, `ws_id`, `email`, `access_token`, `created_at`, `expires_at`, `operation_type`, `is_active`, `rate_limit_per_minute`, `rate_limit_burst`, `rate_limit_window_seconds`, `quota_type`, `total_quota`, `remaining_quota`, `quota_synced_at`, `max_scripts`, `current_scripts`, and `updated_at`.

For `GET /flow/tokens`, queries using only `ws_id` or only `email` mask `access_token` as its first 15 characters plus `***`; queries using `token` or both `ws_id` and `email` return the complete value.

### CodeScript

`id`, `ws_id`, `email`, `description`, `code_base64`, `code_hash`, `code_length`, `version`, `ip_whitelist`, `created_at`, `updated_at`, optional `available_versions`, and `lock` (`is_locked`, `locked_at`, `owner_name`, `owner_display_name`, `owner_email_hint`). `owner_email_hint` is a masked middle segment and never the complete email.
...
### Self-service token queries

- `GET /flow/token/self`: Token authentication; returns `data={count,tokens:[TokenInfo]}` with full access tokens to the authenticated client.
- `POST /flow/token/request-verify-code`: unauthenticated under the global IP limit; body `{ws_id,email}`. When sessions are enabled, the request must include the client-bound `flow_page_session` cookie. Returns a sent-message or rate-limit error.
- `POST /flow/token/verify-and-query`: unauthenticated under the global IP limit; body `{ws_id,email,code}` and the page session when enabled. Returns matching tokens or verification/not-found errors.
...
- `GET /flow/tokens`: query by at least one of `token`, `ws_id`, and `email`; token takes precedence, then `ws_id+email`, then either field alone. Only active tokens are returned and masking follows the query shape.
...
- `POST /flow/scripts/{scriptId}/lock` and `/unlock`: use the matching email/code; lock also requires an owner name of 1-100 non-control characters. Unlock retains owner identity.
- `POST /flow/scripts/{scriptId}/release-ownership`: requires an unlocked owned script and a release code; atomically clears owner data, cancels pending transfers, and records the action.
- `POST /flow/scripts/{scriptId}/ownership-transfers`: authorize with the current owner email or script-creating token registration email, then send a code only to `new_owner_email` and return `transfer_id`.
- `POST /flow/scripts/{scriptId}/ownership-transfers/{transferId}/confirm`: use the new owner email and code to atomically switch ownership while preserving lock state.
...
- `POST /flow/admin/scripts/{scriptId}/ownership-recovery`: emergency `unlock`, `release_owner`, or `assign_owner` actions with a required reason. Administrator tokens do not bypass normal ownership/deletion gates.
