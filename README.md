...
```bash
bunx --bun flow-codeblock-mcp@0.2.20
```
...
```json
{
  "mcpServers": {
    "flow-codeblock": {
      "command": "bunx",
      "args": ["--bun", "flow-codeblock-mcp@0.2.20"],
      "env": {
        "FLOW_CODEBLOCK_BASE_URL": "https://qingcode.oalite.com",
...
- `flow_preview_script_change` 会向模型暴露结构化的 `interface_doc` 工具 Schema。预览前还会纠正可无歧义识别的 `responses/logic_description` 深层错位、误放的完整请求示例、`schema.properties.required`、脚本 `description` 和 `ip_whitelist`，并通过 `interface_doc_normalizations` 返回修正记录；固定字段对象使用 `properties`，仅键名未知且值同构的字典使用 `additionalProperties`。
