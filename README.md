...
```bash
bunx --bun flow-codeblock-mcp@0.2.25
```
...
`FLOW_CODEBLOCK_BASE_URL` 是已部署的 Flow Codeblock Rust API 地址，也是脚本最终调用地址的基址。`flow_apply_script_change` 会返回 `${FLOW_CODEBLOCK_BASE_URL}/flow/codeblock/{script_id}` 形式的 `data.script_url`，无需再向用户询问域名。公网服务使用 `https://qingcode.oalite.com`；仅当用户在本机部署 Rust 服务时才改为对应的 localhost 地址。Token 应使用客户端的环境变量或密钥配置，不要写入提示词、工具参数或公开文件。
...
```json
{
  "mcpServers": {
    "flow-codeblock": {
      "command": "bunx",
      "args": ["--bun", "flow-codeblock-mcp@0.2.25"],
      "env": {
        "FLOW_CODEBLOCK_BASE_URL": "https://qingcode.oalite.com",
...
- `flow_apply_script_change` 发布成功后会在 `data.script_url` 返回由 `FLOW_CODEBLOCK_BASE_URL` 和脚本 ID 拼出的完整调用地址，不要再向用户询问域名。
