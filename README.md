...
```bash
bunx --bun flow-codeblock-mcp@0.2.16
```
...
```json
{
  "mcpServers": {
    "flow-codeblock": {
      "command": "bunx",
      "args": ["--bun", "flow-codeblock-mcp@0.2.16"],
      "env": {
        "FLOW_CODEBLOCK_BASE_URL": "https://qingcode.oalite.com",
...
- 预览前会纠正接口文档中可无歧义识别的 `responses/logic_description` 工具参数错层、`schema` 字段错位、`request.example` 或别名节点示例缺失，并移除 `usage_refs` 中无效的非对象说明；固定字段对象使用 `properties`，仅键名未知且值同构的字典使用 `additionalProperties`，校验错误会去重并按路径提示局部修复；修正记录通过 `interface_doc_normalizations` 返回。
